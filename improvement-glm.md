...
- `npm i -D eslint eslint-config-next prettier vitest`
- Add scripts to `package.json`:
  - `"typecheck": "tsc --noEmit"`
  - `"lint": "eslint ."` (note: `next lint` was removed in Next 16; use ESLint CLI directly with the `eslint-config-next` flat config)
  - `"test": "vitest run"`
  - `"format": "prettier ."`
- Create `eslint.config.mjs` using the flat-config pattern from `node_modules/next/dist/docs/01-app/03-api-reference/05-config/03-eslint.md` (core-web-vitals + typescript presets, ignore `.next`, `out`, `build`, `next-env.d.ts`).
...
**Verify Phase 0:** `npm run typecheck && npm run lint && npm run test && npm run build` all green; CI is live and passing.
...
**Verify Phase 1:** All fixture tests pass; `npm run typecheck && npm run lint && npm run build` clean; `node-sql-parser` upgrade tested by bumping a major version locally and confirming green.
...
- **Phase 2 store migration** touches `page.tsx` and `SchemaGraphCanvas.tsx`; do it after Phase 0 (eslint/format are in place) to keep diffs reviewable.
...
- [ ] Phase 0.3 — Tooling (ESLint/Prettier/vitest/CI)
