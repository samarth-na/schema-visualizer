...
describe('parseSql', () => {
  ...
  describe('multi-column FK constraint', () => {
    it('emits one relationship per column pair', () => {
      ...
      expect(
        result.relationships.some((r) => r.sourceColumn === 'a' && r.targetColumn === 'a')
      ).toBe(true);
      expect(
        result.relationships.some((r) => r.sourceColumn === 'b' && r.targetColumn === 'b')
      ).toBe(true);
    });
  });
  ...
  describe('cross-schema references', () => {
    it('preserves schema names and resolves cross-schema FKs', () => {
      ...
      const publicProfiles = result.tables.find(
        (t) => t.name === 'profiles' && t.schema === 'public'
      );
    ...
    it('parses SAMPLE_SCHEMA with expected tables and relationships', () => {
      ...
      expect(
        rels.some(
          (r) =>
            r.sourceTable === 'posts' &&
            r.sourceColumn === 'author_id' &&
            r.targetTable === 'authors'
        )
      ).toBe(true);
      expect(
        rels.some(
          (r) =>
            r.sourceTable === 'post_tags' &&
            r.sourceColumn === 'post_id' &&
            r.targetTable === 'posts'
        )
      ).toBe(true);
      expect(
        rels.some(
          (r) =>
            r.sourceTable === 'post_tags' && r.sourceColumn === 'tag_id' && r.targetTable === 'tags'
        )
      ).toBe(true);
      expect(
        rels.some(
          (r) =>
            r.sourceTable === 'comments' &&
            r.sourceColumn === 'post_id' &&
            r.targetTable === 'posts'
        )
      ).toBe(true);
