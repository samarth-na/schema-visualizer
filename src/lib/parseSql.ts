...
function isIdentity(definition: unknown): boolean {
  if (!definition || typeof definition !== 'object') return false
  const def = definition as Record<string, any>
  if (def.auto_increment === true || String(def.auto_increment).toLowerCase() === 'true') return true
...
function getInlineReference(definition: unknown): { schema: string; table: string; columns: string[] } | null {
  if (!definition || typeof definition !== 'object') return null
  const def = definition as Record<string, any>
...
function getTableLevelReference(definition: unknown): { schema: string; table: string; columns: string[] } | null {
  if (!definition || typeof definition !== 'object') return null
  const def = definition as Record<string, any>
  const ref = def.reference_definition
  if (!ref) return null
    ...
    if (stmt.type === 'create' && String(stmt.keyword).toLowerCase() === 'table') {
      ...
      for (const def of createDefs) {
        ...
        const constraintName =
          typeof def.constraint === 'string' ? normalizeIdentifier(def.constraint) : `${table}_fkey`
        const sourceCols = extractConstraintColumns(def.definition)
        const refInfo = getTableLevelReference(def)

        if (!refInfo || !refInfo.table || sourceCols.length === 0 || refInfo.columns.length === 0) continue
